...
public final class FPTServer {
    ...
    private FPTServer(@NotNull String host, int port, @NotNull Protocol protocol, @NotNull EventGroup eventGroup, boolean ownedEventGroup, @NotNull Messenger listener) {
        this.host = host;
        this.port = port;
    ...
    public static @NotNull FPTServer run(@NotNull String host, int port, @NotNull Protocol protocol) {
        ...
        FPTServer server = new FPTServer(host, port, protocol, eventGroup, true, Messenger.empty());
        server.start();
        return server;
    }

    public static @NotNull FPTServer run(@NotNull String host, int port, @NotNull Protocol protocol, @NotNull EventGroup eventGroup, @NotNull Messenger listener) {
